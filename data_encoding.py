...
def preprocess(df):
    ...
    # Fill NaN with median or mean before normalizing
    ...
    # target values
    # chat = (df['label'] == 'ChatGPT').astype(int)
    # claude = (df['label'] == 'Claude').astype(int)
    # gemini = (df['label'] == 'Gemini').astype(int)

    # Combine features
    X = np.hstack([academic_norm.values.reshape(-1, 1), subopt_norm.values.reshape(-1, 1),
                   best_tasks_encoded, suboptimal_tasks_encoded, one_hot_encode(df, max_features=50, use_tfidf=True)])
    # y = np.hstack([chat.values.reshape(-1, 1), claude.values.reshape(-1, 1), gemini.values.reshape(-1, 1)])
    y = df['label'].values  # 'ChatGPT', 'Claude', or 'Gemini' strings
...
#     global X_train, y_train, X_test, y_test = X[:n_train], y[:n_train], X[n_train:], y[n_train:]
