"""
Enhanced Random Forest Model Finder with Feature Selection
...
def best_random_forest_model_cv(X_train, y_train, X_val, y_val, cv_folds=5):
    """
    Find the best hyperparameters for Random Forest based on cross-validation and validation accuracy.
    Uses conservative parameters to reduce overfitting on 800 samples.
    """
    ...
    print(f"    Testing {len(n_estimators_values)} n_estimators × {len(max_depth_values)} max_depth × {len(min_samples_split_values)} min_samples_split...")
    print(f"    Total combinations: {len(n_estimators_values) * len(max_depth_values) * len(min_samples_split_values) * len(min_samples_leaf_values) * len(max_features_values)}")
    ...
    if best_model is not None:
        print(f"    Best Random Forest model found:")
        print(f"      n_estimators={best_params['n_estimators']}, max_depth={best_params['max_depth']}")
        print(f"      min_samples_split={best_params['min_samples_split']}, min_samples_leaf={best_params['min_samples_leaf']}")
...
def main_with_feature_selection():
    ...
    """
    print("="*80)
    print("RANDOM FOREST MODEL SEARCH WITH FEATURE SELECTION (800 samples)")
    print("="*80)
        ...
        try:
            ...
            for method in selection_methods:
                for n_features in n_features_to_test:
                    ...
                    # Find best Random Forest model for this configuration
                    model, params = best_random_forest_model_cv(X_train, y_train, X_val, y_val, cv_folds=5)
    ...
    print(f"{'Rank':<5} {'Features':<25} {'Method':<12} {'#Feat':<7} {'Train':<8} {'Val':<8} {'Test':<8} {'Gap':<7} {'Trees':<6}")
    print("-"*110)
    
    for rank, r in enumerate(results_sorted[:15], 1):
        combo_str = '+'.join(r['combo'])
        print(f"{rank:<5} {combo_str:<25} {r['method']:<12} {r['n_features']:<7} {r['train_acc']:.4f}   {r['val_acc']:.4f}   {r['test_acc']:.4f}   {r['overfit_gap']:.4f}  {r['n_estimators']:<6}")
    ...
    if best_overall_model is not None:
        ...
        print(f"  n_estimators={best_overall_config['params']['n_estimators']}")
        print(f"  max_depth={best_overall_config['params']['max_depth']}")
        print(f"  min_samples_split={best_overall_config['params']['min_samples_split']}")
        print(f"  min_samples_leaf={best_overall_config['params']['min_samples_leaf']}")
        print(f"  max_features={best_overall_config['params']['max_features']}")
